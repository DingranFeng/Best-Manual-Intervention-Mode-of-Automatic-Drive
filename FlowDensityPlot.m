...
%% ======================================================================
rMchange = 0.01:0.05:0.7;
flowMaxHist = [];
leg = num2str(rMchange');
for ratioManual = rMchange % 人工干预（手动驾驶）频率
    ...
    flow = zeros(1,Ttotal);
    for i = 1:Ttotal
        density(i) = sum(sum(1-roadHist(i,:,:)))/(nRoad*nLength); % 计算密度
        flow(i) = sum(sum(vHist(i,:,:)))/(nRoad*nLength); % 计算流量
    end
    flowMaxHist = [flowMaxHist,max(flow)];
    plot(density(1:end),flow(1:end),'-.','linewidth',1.4);
    hold off
end
figure(2)
hold on
xlabel('密度/(辆/m)','fontsize',14)
ylabel('流量/(辆/s)','fontsize',14)
legend(leg);
grid on
hold off
...
figure(3)
hold on
plot(rMchange,flowMaxHist,'*-','linewidth',1.8);
xlabel('人工干预频率','fontsize',14)
ylabel('最大车流量','fontsize',14)
grid on
hold off
